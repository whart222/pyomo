...
class ProblemWriter_poeknl(AbstractProblemWriter):

    def __init__(self):

        AbstractProblemWriter.__init__(self, ProblemFormat.poeknl)
    ...
    def __call__(self,
        ...
        if len(io_options):
            raise ValueError(
                "ProblemWriter_poeknl passed unrecognized io_options:\n\t" +
                "\n\t".join("%s = %s" % (k,v) for k,v in iteritems(io_options)))

        poek_model = poek.util.pyomo_to_poek(model)
        fname = output_filename[:-6]+"nl"
...
if poek_available:
    ProblemWriter_poeknl = WriterFactory.register('poeknl', 'Generate the corresponding LP file using POEK')(ProblemWriter_poeknl)
